...
<div class="container">
	...
	<form class="form-horizontal" action="/community/groupboard/update/{boardNo}" method="post">
		<div>
			<!-- 숨겨서 넘길 정보들 -->
		...
		<div class="form-group">
			<div class="col-sm-12" style="text-align: center; padding-bottom: 10px;">
				<button type="button" class="btn btn-lg btn-default" id="likeBtn" value="N" onclick="likeGBoard(this.form)">
				...
				<button type="button" class="btn btn-primary" onclick="location.href='/community/groupboard/${detail.groupNo}'">목록</button>
			</div>
		</div>
	</form>
	<!-- 댓글 영역 -->
	...
	$(document).ready(function() {
		...
		// 게시글--------------------------------------------------
		// 좋아요 누른 게시글인지 확인
		checkGbLikes($("#boardNo").val(), $("#loginId").val());
		// 조회수를 확인해서 알림을 보낸다.
		notiToGbViews($("#views").val());
		
		// 내용 수정 버튼이 눌렸을 경우
		$("#updateBtn").on("click", function() {
			// 작성자와 로그인한 아이디가 같은지 확인
			if(document.getElementById("writer").value == document.getElementById("loginId").value){
				location.href="/community/groupboard/update/${detail.boardNo}";
			} else {
		...
		$("#deleteBtn").on("click", function() {
			...
			if(document.getElementById("writer").value == document.getElementById("loginId").value){
				if(confirm("정말 삭제하시겠습니까?") == false){
					...
					location.href="/community/groupboard/delete/${detail.boardNo}";
				}
			} else {
		...
		$("#commentInsertBtn").on("click", function() {
			gbcommentInsert($("#commentContent").val());
		});
		// 댓글창에서 엔터키를 입력할 경우
