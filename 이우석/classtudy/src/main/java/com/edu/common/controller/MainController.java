...
public class MainController {
	...
	public String main(Model model, HttpSession session) throws Exception {
		...
		if (hitDate == null) {
		    hitDate = "없음"; }
		logger.info("MainController 오늘날짜....." + hitDate);
		// 방문자수
	...
	// 태그별 게시판 게시글 검색(제목 또는 내용에 키워드가 들어간 글 찾기)
	...
	private String search(@PathVariable int searchCode, @PathVariable String keyword, @PathVariable String tags,
			@PathVariable Optional<Integer> pageNum, Model model, HttpServletRequest request) throws Exception {
		logger.info("MainController search()....");
		logger.info("MainController search() : " + keyword);
		logger.info("MainController search() : " + tags);
		// 값이 POST로 넘어올 때 선택된 tag의 value를 저장한다.
				if (request.getParameter("tags") != null) {
					tags = request.getParameter("tags");
				} 
