...
/*	function checkjoin(groupNo) {
	...
	function commentUpdate(commentNo, content, boardNo, indexNo) {
		...
		if(confirm("수정 하시겠습니까?") == false) {
			...
			str += '<span class="input-group-btn"><button class="btn btn-warning" type="button" onclick="commentUpdateProc('+commentNo+',\''+boardNo+'\');">수정</button> </span>';
			str += '</div>';
			$('.commentContent' +commentNo).html(str);
		}
	}
	//댓글 수정 - 수정한 댓글 내용을 테이블에 업데이트한다.
	function commentUpdateProc(commentNo, boardNo) {
		//댓글 번호에 해당하는 수정된 내용을 가져온다.
		alert("boardNo : " + boardNo + "commentNo : " + commentNo );
			...
			success: function(data) {
				...
				if (data.length < 1) { //댓글이 없을 때
					...
					$.each(data,function(key, value){ 
						...
						str += '<div class="commentContent' + value.commentNo + '" align="left" id="Ccm"><p>' + value.content +'</p></div><div></div><div></div>';
						if(value.writer == uid){
