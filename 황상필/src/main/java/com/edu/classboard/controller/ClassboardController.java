...
import com.edu.common.service.PointService;
...
public class ClassboardController {
	...
	private String postWrite(ClassboardDTO cbDTO, HttpSession session, Model model) throws Exception {
		...
		// 오늘날짜 TIL 작성에 따른 포인트 지급이 있었는지 확인하는 작업을 서비스에게 의뢰한다.
		int TILPointCheck = pointService.isTILPointCheck(memberId);
		
		// 오늘날짜 TIL 작성에 따른 포인트 지급이 있었다면 count값이 0이상이 나오므로 바로 클래스보드 리스트로 돌려보낸다.(즉 이미 TIL 작성 포인트를 지급받았음)
		if(TILPointCheck > 0) {
			return "redirect:/class/classboard/all";
		} else { // 오늘날짜 TIL에 따른 포인트 지급이 없다면 아래와 같은 내용과 함께 1포인트 지급해준다.
			String TILPointContent = "TIL을 작성하여 1포인트가 지급되었습니다.";				
			
			pointService.addLoginPoint(memberId, TILPointContent);
		}				
	...
	private String list(@PathVariable String viewCategory, @PathVariable Optional<Integer> pageNum, 
		...
		logger.info("ClassboardController list() viewCategory : " + viewCategory);
		 ...
		 if (lectureNo == 1 || lectureNo == 0) { model.addAttribute("message",
		 "수강중인 강의가 있어야 이용할 수 있는 메뉴입니다.<br>관리자 확인을 받은 후 다시 이용해주세요."); 
