...
public class MemberController {
	...
	private String login(MemberDTO memberDTO, HttpServletRequest req, RedirectAttributes rttr) throws Exception {
		...
		if(login == null) {
			...
			// 오늘날짜 login에 따른 포인트 지급이 있었는지 확인하는 작업을 서비스에게 의뢰한다.
			int loginPointCheck = pointService.isLoginPointCheck(memberId);
			
			// 오늘날짜 login에 따른 포인트 지급이 있었다면 count값이 0이상이 나오므로 바로 메인페이지로 돌려보낸다.(즉 이미 login 포인트를 지급받았음)
			if(loginPointCheck > 0) {
				return "redirect:/member/login";
			} else { // 오늘날짜 login에 따른 포인트 지급이 없다면 아래와 같은 내용과 함께 1포인트 지급해준다.
				String loginPointContent = "로그인을 성공하여 1포인트가 지급되었습니다.";				
				
				pointService.addLoginPoint(memberId, loginPointContent);
			}				
